...
данной платы представлена на рисунке ниже. В текущей версии используется пин 7 для
подключения реле управления пневмоотталкивателем. Пин 13 -- встроенный светодиод.
...
Функция **void setup()** используется для однократного выполнения действий инициализации при
запитывании платы контроллера.

Функция **void loop()** используется для программирования действий, которые будут выполняться
микроконтроллером циклично (постоянно), пока плата запитывается.

Определение режима работы пина производится с использованием функции **pinMode**.

Запись логической единицы либо нуля в соответствующий пин осуществляется **digitalWrite**.
