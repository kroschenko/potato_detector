...
class TestIntegration(unittest.TestCase):
    ...
    def run_test_case(self, test_config):
        ...
        if not os.path.exists(MainConfigs.POTATO_DETECTOR_PATH):
            raise FileNotFoundError(f"Potato detector model not found at: {MainConfigs.POTATO_DETECTOR_PATH}")
        if not os.path.exists(MainConfigs.DEFECTS_DETECTOR_PATH):
            raise FileNotFoundError(f"Defects detector model not found at: {MainConfigs.DEFECTS_DETECTOR_PATH}")
        ...
        self.tracker = PotatoTracker(MainConfigs.CAMERA_FRAME_SHAPE, 
                                   self.potato_defects_queue, 
                                   self.potato_timing_queue)
        ...
        while frame_count < test_config.max_frames:
            ...
            if frame_count % 50 == 0:
                ...
                logger.info(f"Current unique potatoes: {self.tracker.get_total_objects_count()}")
                logger.info(f"Current total defects: {self.tracker.total_defects_detected}")
        ...
        if test_config.test_type in ["total", "all"] and test_config.expected_potatoes is not None:
            self.assertEqual(total_potatoes, test_config.expected_potatoes, 
                           f"Expected {test_config.expected_potatoes} potatoes, got {total_potatoes}")
        
        if test_config.test_type in ["defects", "all"] and test_config.expected_defects is not None:
            self.assertEqual(total_defects, test_config.expected_defects,
        ...
        if total_potatoes > 0 and test_config.expected_potatoes is not None and test_config.expected_defects is not None:
    ...
    def test_potato_detection(self):
        ...
        for test_config in TEST_CASES:
            with self.subTest(video=test_config.video_path, test_type=test_config.test_type):
