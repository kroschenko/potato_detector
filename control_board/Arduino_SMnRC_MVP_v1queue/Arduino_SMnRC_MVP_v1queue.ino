...
uint16_t duration_nozzle_open = 120; // duration of open nozzle 363
...
unsigned long nozzle_delay_before_open[nozzle_relays] = {420,600}; // 610 875
bool laser_state = 0;
...
bool update_nozzle_delays(const String& input) {
  int separator = input.indexOf(',');
  if (separator <= 1) {
    return false;
  }
  unsigned long top_delay = input.substring(1, separator).toInt();
  unsigned long bottom_delay = input.substring(separator + 1).toInt();
  nozzle_delay_before_open[0] = top_delay;
  nozzle_delay_before_open[1] = bottom_delay;
  Serial.print("{\"D\":[");
  Serial.print(nozzle_delay_before_open[0]);
  Serial.print(",");
  Serial.print(nozzle_delay_before_open[1]);
  Serial.println("]}");
