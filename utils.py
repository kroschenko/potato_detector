...
from camera import AVICamera, DO3ThinkCamera, OpenCVCamera
...
def log_camera_info(camera):
    ...
    if isinstance(camera, AVICamera):
        ...
        if camera.device is not None and camera.device.isOpened():
            ...
            logger.debug(f"  - Frame Width: {camera.device.get(cv2.CAP_PROP_FRAME_WIDTH)}")
            logger.debug(f"  - Frame Height: {camera.device.get(cv2.CAP_PROP_FRAME_HEIGHT)}")
            ...
            logger.debug(f"  - Current Position: {camera.device.get(cv2.CAP_PROP_POS_FRAMES)}")
            logger.debug(f"  - Brightness: {camera.device.get(cv2.CAP_PROP_BRIGHTNESS)}")
            logger.debug(f"  - Contrast: {camera.device.get(cv2.CAP_PROP_CONTRAST)}")
            logger.debug(f"  - Saturation: {camera.device.get(cv2.CAP_PROP_SATURATION)}")
        ...
        logger.debug(f"  - Camera Name: {camera.cam_descriptor}")
        ...
        logger.debug(f"  - Camera Index: {camera.cam_descriptor}")
        logger.debug(f"  - Device Activated: {camera.device_is_activated()}")
