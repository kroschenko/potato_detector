...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f"jpg_to_avi_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def verify_video_file(video_path, expected_frames):
    ...
    try:
        ...
        if frame_count != expected_frames:
            return False, f"Frame count mismatch: expected {expected_frames}, got {frame_count}"
        
        return True, f"Video verified: {frame_count} frames, {width}x{height} resolution"
    except Exception as e:
        return False, f"Verification error: {str(e)}"

def create_video_from_jpgs(input_dir, output_file, start_frame=0, num_frames=None, fps=30,
                          codec='xvid', quality=100, resume=False, resize=None,
    ...
    try:
        ...
        processed_frames = 0
        for i, frame_file in enumerate(tqdm(jpg_files[current_frame:], desc="Creating video")):
        ...
        if os.path.exists(output_file):
            try:
                ...
                logger.error(f"Failed to remove incomplete video file: {str(cleanup_error)}")
        raise

def main():
    parser = argparse.ArgumentParser(description='Convert JPG images to AVI video')
    parser.add_argument('--input_dir', type=str, default='/home/user/frames',
                        help='Directory containing JPG images')
    parser.add_argument('--output_file', type=str, default='output.avi',
